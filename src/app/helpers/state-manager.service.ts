...
export class StateManager {
  ...
  public initialiseStorageSyncListener() {
    if (StateManager.syncListenerInitialised === true) {
      return;
    }

    StateManager.syncListenerInitialised = true;
  ...
  public deinitialiseStorageSyncListener() {
    ...
    StateManager.syncListenerInitialised = false;
  }
  ...
  public clearAllSessionsStorage() {
    ...
    localStorage.removeItem(StateManager.DBKEY_SYNC_KEYS);
  ...
  public saveSessionData(data: any, key = StateManager.DBKEY_USER_DATA) {
  ...
  public saveSyncedSessionData(data: any, key = StateManager.DBKEY_USER_DATA) {
    this.testForInvalidKeys(key);
  ...
  public savePermanentData(data: any, key = StateManager.DBKEY_USER_DATA) {
    this.testForInvalidKeys(key);
  ...
  public moveDataToSessionStorage(key = StateManager.DBKEY_USER_DATA) {
    this.testForInvalidKeys(key);
  ...
  public moveDataToSyncedSessionStorage(key = StateManager.DBKEY_USER_DATA) {
    this.testForInvalidKeys(key);
  ...
  public moveDataToPermanentStorage(key = StateManager.DBKEY_USER_DATA) {
    this.testForInvalidKeys(key);
  ...
  public exists(key = StateManager.DBKEY_USER_DATA) {
  ...
  public getData(key = StateManager.DBKEY_USER_DATA) {
    this.testForInvalidKeys(key);
  ...
  public getDataObject<T>(key = StateManager.DBKEY_USER_DATA, isDateType = false): T {
  ...
  public deleteData(key = StateManager.DBKEY_USER_DATA) {
    this.testForInvalidKeys(key);
  ...
  private getSyncKeysFromStorage(defaultValue: string[] = []): string[] {
    const data = this.localStorageGetItem(StateManager.DBKEY_SYNC_KEYS);
  ...
  private addToSyncKeysBackup(key: string) {
    ...
    if (!storedSyncKeys.some(x => x === key)) {
      ...
      this.localStorageSetItem(StateManager.DBKEY_SYNC_KEYS, storedSyncKeys);
    }
  }
