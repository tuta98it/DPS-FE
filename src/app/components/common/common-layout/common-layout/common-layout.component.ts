...
export class CommonLayoutComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this._currentCaseSubscription = this.viewerState.subscribeCurrentCase(
            (tab: any) => {
                // console.log('subscribeCurrentCase here', tab);
                if (
                    tab != null &&
                    tab.caseStudyId != undefined &&
                    tab.caseStudyId != 'null'
                ) {
        ...
        this._notificationSub = this.firebaseService.currentMessage.subscribe(
            (message: any) => {
                console.log(message);
    ...
    onFirebaseMessage(message: any) {
        if (message) {
            ...
            let state =
                message.data.type == Constants.UPLOAD_PROCESS_TYPE.PROCESS_DONE
            ...
            if (state == Constants.UPLOAD_STATUS.COMPLETED) {
                if (this.layoutConfig == this.LAYOUT_CONFIG.DEFAULT) {
                    this.worklist.onCaseStudyAction({
                    ...
                    if (!this.VTWorklist.loading) {
                        this.VTWorklist.onCaseStudyAction({
                            action: Constants.CASE_STUDY_ACTIONS.REFRESH,
                        });
                    }
                }
    ...
    onToggleShowSupportLabeling(data: any) {
        this.isShowSupportLabeling = !this.isShowSupportLabeling;
    }
}
