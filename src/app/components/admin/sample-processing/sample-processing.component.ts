...
export class SampleProcessingComponent implements OnInit {
    ...
    saveItem() {
        if (this.sampleProcessingForm.valid) {
            ...
            Object.values(this.sampleProcessingForm.controls).forEach(
                (control) => {
                    if (control.invalid) {
                        control.markAsDirty();
    ...
    updateSampleProcessing() {
        this.sampleProcessingService
            .update(
                this.sampleProcessingForm.value.id,
                this.sampleProcessingForm.value
            )
            .subscribe({
                next: (res) => {
                    if (res.isValid) {
    ...
    createSampleProcessing() {
        this.sampleProcessingService
            .create(this.sampleProcessingForm.value)
            .subscribe({
                next: (res) => {
                    if (res.isValid) {
