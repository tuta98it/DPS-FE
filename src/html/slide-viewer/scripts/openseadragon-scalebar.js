...
(function($) {
    ...
    $.Scalebar = function(options) {
        ...
        this.hatchMark = options.hatchMark || 10;
        this.numMark = options.numMark || 10;
        this.direction = options.direction || $.ScalebarDirection.HORIZONTAL;
    ...
    $.Scalebar.prototype = {
        updateOptions: function(options) {
            ...
            if (isDefined(options.hatchMark)) {
                this.hatchMark = options.hatchMark;
            }
            if (isDefined(options.numMark)) {
                this.numMark = options.numMark;
            }
        },
        setDrawScalebarFunction: function(type) {
            ...
            else if (type === $.ScalebarType.RULER) {
            	this.drawScalebar = this.drawRulerScalebar;
                // if ruler locate on the middle left
            	if (this.location == $.ScalebarLocation.LEFT_MIDDLE) {
                    this.divElt.className += 'ruler-left';
                    // divElt sẽ gồm 3 hàng
                    // hàng 1: kích thước
                    // hàng 2: vạch
                    // hàng 3: các số từ 1 đến 10
                    //this.divElt.appendChild();
                    //
                    let rulerLine = document.createElement('div');
                    rulerLine.className += 'ruler-line';
                    //console.log(this.h)
                    //console.log(this.hatchMark);
                    this.hatchMark = this.hatchMark || 10;
                    for (let i = 0; i < this.hatchMark; i++) {
                        rulerLine.appendChild(document.createElement('div'));
                    }
                    this.numMark = this.numMark || 10;
                    let rulerNumber = document.createElement('div');
                    rulerNumber.className += 'ruler-number';
                    for (let i = 0; i < this.numMark; i++) {
                        let td = document.createElement('div');
                        //td.innerHTML = (i + 1);
                        rulerNumber.appendChild(td);
                    }

                    // let 
                    // smolPotat : 1 div nhỏ chứa thước và số
                    let smolPotat = document.createElement('div');
                    smolPotat.appendChild(rulerLine);
                    smolPotat.appendChild(rulerNumber);

                    this.divElt.appendChild(document.createElement('div'));
                    this.divElt.appendChild(smolPotat);
                    
                } else {
                    this.divElt.className += 'ruler';
                    // divElt sẽ gồm 3 hàng
                    // hàng 1: kích thước
                    // hàng 2: vạch
                    // hàng 3: các số từ 1 đến 10
                    ...
                    this.divElt.appendChild(document.createElement('div'));
                    this.divElt.appendChild(rulerLine);
                    this.divElt.appendChild(rulerNumber);
                }
        ...
        setMinWidth: function(minWidth) {
            ...
            this.minWidth = this.divElt.offsetWidth || parseInt(minWidth);
        },
        /**
        ...
        drawRulerScalebar: function(size, text) {
            this.divElt.style.fontSize = this.fontSize;
            this.divElt.style.fontFamily = this.fontFamily;
            this.divElt.style.textAlign = "right";
            this.divElt.style.color = this.fontColor;
            // this.divElt.style.border = this.barThickness + "px solid " + this.color;
            this.divElt.style.borderTop = "none";
            this.divElt.style.backgroundColor = this.backgroundColor;
            this.divElt.firstElementChild.innerHTML = text;
            //console.log(text);
            // if this ruler is vertical
            if (this.location == $.ScalebarLocation.LEFT_MIDDLE) {
                this.divElt.className = 'ruler-vertical';
                this.divElt.style.height = size + 'px';
            } else {
                this.divElt.style.width = size + "px";
            }
            
            // Set number on the ruler
            let length = parseFloat(text);
            let skiped = false;
            if (length > 100) {
                skiped = true;
            }
            let ruler = this.divElt.children[2] || this.divElt.children[1].children[1];
            for (let i = 0; i < this.numMark; i++) {
                if (!ruler.children[i]) continue;
                if (skiped && !(i & 1)) {
                    ruler.children[i].innerHTML = "";
                    continue;
                }
                ruler.children[i].innerHTML = (length / this.numMark * (i + 1)).toFixed(1);
            }
        },
        /**
        ...
        getScalebarLocation: function() {
            ...
            if (this.location === $.ScalebarLocation.BOTTOM_MIDDLE) {
                ...
                var barHeight = this.divElt.offsetHeight;
                var container = this.viewer.container;
                var x = container.offsetWidth / 2 - barWidth / 2;
                var y = container.offsetHeight - barHeight;
                if (this.stayInsideImage) {
                    var pixel = this.viewer.viewport.pixelFromPoint(
                            new $.Point(0, 1 / this.viewer.source.aspectRatio),
                ...
                return new $.Point(x, y - this.yOffset);
            }
             if (this.location === $.ScalebarLocation.LEFT_MIDDLE) {
                var barWidth = this.divElt.offsetWidth;
                var barHeight = this.divElt.offsetHeight;
                var container = this.viewer.container;
                var x = 0;
                var y = container.offsetHeight / 2 - barHeight / 2;
                if (this.stayInsideImage) {
                    var pixel = this.viewer.viewport.pixelFromPoint(
                            new $.Point(0, 1 / this.viewer.source.aspectRatio),
                            true);
                ...
                return new $.Point(x + this.xOffset, y);
            }
        },
        /**
